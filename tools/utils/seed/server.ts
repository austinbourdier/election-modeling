...
import { COVERAGE_PORT, DOCS_DEST, DOCS_PORT, PORT, PROD_DEST } from '../../config';
...
export function serveDocs() {
  ...
  server.listen(DOCS_PORT, () =>
    openResource('http://localhost:' + DOCS_PORT + '/app')
  );
}
...
export function serveCoverage() {
  ...
  server.listen(COVERAGE_PORT, () =>
    openResource('http://localhost:' + COVERAGE_PORT + '/app')
  );
}
...
export function serveProd() {
  ...
  server.use('/app', serveStatic(root));

  server.use(fallback('index.html', { root }));

  server.listen(PORT, () =>
    openResource('http://localhost:' + PORT + '/app')
  );
};
